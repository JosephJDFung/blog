...
GC收集算法是内存回收的方法论，垃圾收集器是内存回收的具体实现,垃圾收集器都是一组一组的工作。


![picture](./img/ljsjq.JPG)
...
CMS收集器是基于“标记—清除”算法实现的，它的运作过程相对于前面几种收集器来说更复杂一些
- 整个过程分为4个步骤：
...
-XX:+UseParallelGC|Parallel Scavenge  并行回收GC|Serial Old 串行GC
...
-XX:+UseConcMarkSweepGC-XX:+UseParNewGC|Serial 串行GC|CMS 并发GC 当出现“Concurrent Mode Failure”时
采用Serial Old 串行GC
...
-XX:LargePageSizeInBytes|内存页的大小，设置过大会影响Perm的大小；-XX:LargePageSizeInBytes=128m
