...
Dubbo 采用全 Spring 配置方式，透明化接入应用，对应用没有任何 API 侵入，只需用 Spring 加载 Dubbo 的配置即可，Dubbo 基于 Spring 的 Schema 扩展 进行加载。如果不想使用 Spring 配置，而希望通过 API 的方式进行调用，可以使用API配置。
...
- 所有配置最终都将转换为 URL (protocol://username:password@host:port/path?key=value&key=value) 表示，并由服务提供方生成，经注册中心传递给消费方，各属性对应 URL 的参数

- 常见配置
    - [dubbo:service](###dubbo:service)
    - [dubbo:reference](###dubbo:service)
    - [dubbo:protocol](###dubbo:service)
    - [dubbo:registry](###dubbo:service)
    - [dubbo:monitor](###dubbo:service)
    - [dubbo:application](###dubbo:service)
    - [dubbo:module](###dubbo:service)
    - [dubbo:provider](###dubbo:service)
    - [dubbo:consumer](###dubbo:service)
    - [dubbo:method](###dubbo:service)
    - [dubbo:argument](###dubbo:service)
    - [dubbo:parameter](###dubbo:service)
    - [dubbo:config-center](###dubbo:service)
